function NPV = fMLIAM08(ctax,coth,reg)
...
for t=StartYear+1:EndYear,
    ...
    if t >= 1960,
        output(i,:) = max(0.3*population(i,:),CobbDouglas(TFP(i,:),capital(i,:),population(i,:),lambda).*(1-relabcost(i-1,:)+impacta(i-1,:)/100)); %explicit adaptation
        %output(i,:) = max(0.3*population(i,:),CobbDouglas(TFP(i,:),capital(i,:),population(i,:),lambda).*(1-relabcost(i-1,:)+impact(i-1,:)/100)); %implicit adaptation
        ...
        [adaptation(i,:), impacta(i,:), impact(i,:)] = impactcca(atmtemp(i),adaptpar);
    end
